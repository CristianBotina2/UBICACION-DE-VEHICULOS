...
int main(void) {
  ...
  */
  ...
  chprintf( (BaseChannel *)&SD3, "comenzo %x\n\r");
  /*
  ...
  chprintf( (BaseChannel *)&SD3, "shot ya %x\n\r", PAL_LOW);
  ...
  while(palReadPad(GPIOC, 9) == PAL_LOW){
	...
	chprintf( (BaseChannel *)&SD3, "No While %s\n\r");
	//gptStartContinuous(&GPTD1, 10)
	while(palReadPad(GPIOC, 9) == PAL_HIGH){
		//chprintf( (BaseChannel *)&SD3, "se puso en alto, comienza a contar %s\n\r");
